...
async function handleOutbid(req, res) {
...
module.exports = async (req, res) => {
  ...
  try {
    const { kind } = req.body;
    if (kind === 'outbid') return await handleOutbid(req, res);
    if (kind === 'price_drop') return await handlePriceDrop(req, res);
    res.status(400).json({ error: 'Невідомий тип події' });
  } catch (err) {
